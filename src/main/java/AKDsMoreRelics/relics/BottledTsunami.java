...
import AKDsMoreRelics.patches.BottledTsunamiField;
...
public class BottledTsunami extends CustomRelic implements CustomBottleRelic, CustomSavable<Integer> {
    ...
    // ID, images, text.
    public static final String ID = DefaultMod.makeID("BottledTsunami");
    private static final Texture IMG = TextureLoader.getTexture(makeRelicPath("BottledTsunami.png"));
    private static final Texture OUTLINE = TextureLoader.getTexture(makeRelicOutlinePath("BottledTsunami.png"));
    ...
    public Predicate<AbstractCard> isOnCard() {
        return BottledTsunamiField.inBottledTsunamiField::get;
    }
    ...
    public void onLoad(Integer cardIndex) {
        ...
        if (cardIndex >= 0 && cardIndex < AbstractDungeon.player.masterDeck.group.size()) {
            ...
            if (card != null) {
                BottledTsunamiField.inBottledTsunamiField.set(card, true);
    ...
    public void onUnequip() { // 1. On unequip
        if (card != null) { // If the bottled card exists (prevents the game from crashing if we removed the bottled card from our deck for example.)
            ...
            if (cardInDeck != null) {
                BottledTsunamiField.inBottledTsunamiField.set(cardInDeck, false); // In our SpireField - set the card to no longer be bottled. (Unbottle it)
            }
        }
    }
    ...
    public void update() {
        ...
        if (!cardSelected && !AbstractDungeon.gridSelectScreen.selectedCards.isEmpty()) {
            DefaultMod.logger.info(AbstractDungeon.gridSelectScreen.selectedCards.size());
            ...
            BottledTsunamiField.inBottledTsunamiField.set(card, true); // Use our custom spire field to set that card to be bottled.
    ...
    public void onUseCard(AbstractCard targetCard, UseCardAction useCardAction) { // Whenever we use any card
        if (BottledTsunamiField.inBottledTsunamiField.get(targetCard)) {
